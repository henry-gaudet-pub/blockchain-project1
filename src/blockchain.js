...
const five_minutes = 1000 * 1000 * 60 * 60 * 5;
const star_registry = "star_registry";
...
class Blockchain {
    ...
    async initializeChain() {
        if (this.height === -1) {
            ...
            await this._addBlock(block).catch(() => { console.log('initializeChain failed') });
        }
    }
                ...
                .then((error_log) => {
                    if (error_log.length === 0) {
                        try {
                            ...
                            console.log(`_addBlock failed: ${err}`);
                            reject();
                        }
                    } else {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            let message = address + ':' + timestamp + ':' + star_registry;
            resolve(message);
        });
    }
